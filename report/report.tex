...
A document server parses an uploaded PDF, PPT, word or transcript file and extracts 
embedded images. Any other input format needs a new tool in server. An assessment
server checks whether piece of generated code matches the expected calling code schema.
A storage server saves the finished module to database and reads/writes the vector store.
The pipeline and tutor never import a parsing library/database driver or vector store's 
client directly as each server reaches through same client
...
The full explanation is embedded and saved into per module collection in vector
database through storage server after a topic is enriched.A small embedding model 
runs locally than a separate provider call. The vector store is 
system of record for actual text of each concept. When the pipeline's own content 
for current topic is missing, then tutor's hint and concept presentation query this store.
...
There are four places where system can generate questions. End of module quiz, 
comprehensive questions, pre lesson diagnostic and  tutor’s follow up questions.
The systems has blooms six level cognitive levels: remember, understand, apply, 
analyse, evaluate and create. The level of easy, medium, hard are under these bloom levels.
The end of module has all questions from six levels.The tutor follow up questions maps to the
user understanding depth. The end of module quiz is timed. The timer decrements as on expiry,

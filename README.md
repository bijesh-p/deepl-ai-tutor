...
- **Dark Theme** — The app ships dark-themed by default. Dark mode is applied globally at startup (before the first page renders) so there is no flash of a light theme. There is no runtime theme toggle — dark is the single application default for all pages including login. Implemented via CSS injection in `frontend/styles.py`.

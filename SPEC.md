...
> **Version:** 0.32 | **Last updated:** 2026-06-23
...
| Module Chatbot (Phase 67) | A dedicated chatbot page accessible from the sidebar. The chatbot has three capabilities: **(1) Module discovery** — answers "what modules/topics are available?", "which module covers X?" by listing modules with their topic titles and summaries from the DB; **(2) Content Q&A** — answers specific questions grounded in training content by querying ChromaDB across the user's modules, with source citations; **(3) Learning guidance** — recommends which module to study for a given concept by matching the query against module/topic metadata. The chatbot is aware of all module titles, topic titles, and summaries (loaded from `module_json` in the DB) so it can answer meta-questions without needing ChromaDB hits. Out-of-scope queries (no matching module content) get a clear "no training module covers this" response. Session-scoped conversation history (last 10 turns passed as multi-turn context to the LLM, enabling follow-up questions). Each user's chatbot only searches their own modules. | ✅ Done |
| Module Chatbot fixes (Phase 68) | Five fixes to the chatbot: **(1)** Conversation history now sent to the LLM — `ask()` accepts a `history` parameter, formats the last 10 turns, and includes them in the prompt so follow-up questions ("tell me more", "which topic?") work; **(2)** `query_vector_db` now returns `metadatas` from ChromaDB (was omitted from the JSON response), enabling source enrichment; **(3)** Source `title`/`topic_title` fields populated from ChromaDB metadatas instead of always being empty strings; **(4)** Empty "Retrieved Content" section no longer shown when only the catalog matches (meta-questions like "what modules are available?" get a cleaner prompt); **(5)** Relevance threshold raised from 1.4 to 1.8 to reduce over-filtering of moderately related content. | ✅ Done |
...
- [x] Module Chatbot answers content questions grounded in training modules, lists available modules/topics on request, and recommends which module to use for a given concept; out-of-scope queries flagged clearly

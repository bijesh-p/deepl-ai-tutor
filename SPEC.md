...
> **Version:** 0.14 | **Last updated:** 2026-06-20
...
- [x] **Sidebar can't be re-expanded once collapsed (Phase 45/46)** — Confirmed via a vanilla Streamlit app (same installed version, zero custom CSS) that `[data-testid="stSidebarCollapseButton"]` is `visibility:hidden` by default and never becomes visible/reachable via hover at any point along the page edge, with or without this app's styling — a Streamlit 1.58.0 framework-level issue, not caused by the dark-mode CSS. **The "try a Streamlit version bump" option is closed out as not viable**: `pyproject.toml` already allows any version ≥1.58.0, and 1.58.0 (installed) is the current latest stable release (2026-05-28, confirmed via PyPI) — there is no newer version to upgrade to. Research across Streamlit's GitHub issues and community forum shows this is a recurring, not-version-specific class of bug in the sidebar collapse control going back to at least 1.25/1.38 (the testid itself was renamed `collapsedControl` → `stSidebarCollapseButton` in 1.38 — this app's CSS was still targeting the old, dead name, confirmed and removed in Phase 46). **Remaining options**: live with it, or build a custom JS workaround (e.g. via `streamlit.components.v1.html` with `window.parent.document` access) to add an always-visible toggle that calls the hidden button's `.click()` — not yet implemented, no specific plan to build it unless requested.
